...
public class AlgoTopPHUI {
    ...
    public AlgoTopPHUI(int top, int minPer, int maxPer, int minAvg, int maxAvg, boolean debug) {
        ...
        this.debug = debug;
        if (debug) {
            System.out.println("topkstatic的值为： " + topkstatic);
    ...
    public void runAlgorithm(String input, String output, boolean eucsPrune) throws IOException {
            ...
            while ((thisLine = myInput.readLine()) != null) {
                ...
                for (int i = 0; i < items.length; i++) {
                    ...
                    /*
                     ...
                     在计算最小周期性时，舍去项集的第一个和最后一个周期，因为当项集出现在数据库中的第一个事务中，那么它的周期中必有1，
                     如果项集出现在数据库中的最后一个事务中，那么它的周期中必有0；这将会影响最小周期性的判断*/
        ...
        supportPruningThreshold = (((double) databaseSize) / ((double) maxAveragePeriodicity)) - 1d;// 最大周期性评判指标
        supportPruningThresholdminper = (((double) databaseSize) / ((double) minAveragePeriodicity)) - 1d;// 最小周期性评判指标
        ...
        if (debug) {
            ...
            //System.out.println("piuRaiseValue: " + piuRaiseValue);
        ...
        for (Integer item : mapItemToItemInfo.keySet()) {
            ...
            if (itemInfo.support >= supportPruningThreshold && // satisfy the maximum average period prune condition
                ...
                listOfUtilityLists.add(uList); // 效用列表的列表，保存的是满足 周期性剪枝和TWU剪枝条件的项 的效用列表
                ///*************** PHM ****************
                // set the periodicity
        ...
        if (EUCS_PRUNE) {
            ...
            if (debug) {
                System.out.println("raising PCUD: " + minUtility + ", top-k: " + topkstatic);
                PCUDRaiseValue = minUtility;
                //System.out.println("PCUDRaiseValue: " + PCUDRaiseValue);
                //System.out.println("====================================");
                //System.out.println("minUtility: " + minUtility);
            }
        ...
        // PRU: raising threshold use the real utility of single item and 2-itemSets that satisfy the period condition.
        if (true) {
            ...
            for (Entry<Integer, Map<Integer, Item>> entry : mapFMAP.entrySet()) {
                ...
                Long singleItem = PIU.get(a); // get U(a)
                if (singleItem != null) {
                    ItemInfo itemInfo = mapItemToItemInfo.get(a);
                    if (itemInfo.support >= supportPruningThreshold &&
                        ...
                        value = singleItem;
                        if (value >= minUtility) {
                            if (ktopls.size() < topkstatic)
                                ktopls.add(value);
                            else if (value > ktopls.peek()) {
                                ktopls.add(value);
                                do {
                                    ktopls.poll();
                                } while (ktopls.size() > topkstatic);
                            }
                        }
                    }
                }
                ...
                for (Entry<Integer, Item> entry2 : entry.getValue().entrySet()) {
                    ...
                    if (itemSetInfo.support >= supportPruningThreshold &&
                            itemSetInfo.support <= supportPruningThresholdminper &&
                            itemSetInfo.largestPeriodicity <= maxPeriodicity &&
                            itemSetInfo.smallestPeriodicity >= minPeriodicity) {

                        value = itemSetInfo.utility;
                        if (value >= minUtility) {
                            if (ktopls.size() < topkstatic)
                                ktopls.add(value);
                            else if (value > ktopls.peek()) {
            ...
            if ((ktopls.size() > topkstatic - 1) && (ktopls.peek() > minUtility))
                minUtility = ktopls.peek();

            ktopls.clear();
            if (debug) {
                System.out.println("raising PRU: " + minUtility + ", top-k: " + topkstatic);
                //System.out.println("PRURaiseValue: " + minUtility);
            }
        }

        System.out.println("=========================");
        //System.out.println("PRU: minUtility" + minUtility);
...
//	 */
    ...
    private UtilityList construct2(UtilityList P, UtilityList px, UtilityList py) {
        ...
        for (Element ex : px.elements) {//elements: [tid, iutils, rutils]
...
//		}
    ...
    private Element findElementWithTID(UtilityList ulist, int tid) {
        // 在py的效用列表中，找到与ex.tid相同的tid, 也就是ex.tid = py中的一个tid
        // 其中UtilityList ulist表示py的效用列表，   int tid表示ex.tid
        List<Element> list = ulist.elements;// elements中存储的是[tid, iutils, rutils]，这里的list表示py所出现的事务的tid
...
//	}
    ...
    /**
     * raising threshold CUD optimize : 2-itemSets
     *
     * @param k : top-k
     */
